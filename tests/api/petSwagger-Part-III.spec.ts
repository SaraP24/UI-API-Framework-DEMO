...
test('Add pet', async () => {
  ...
  if (res.status() !== 200) errorMessage("petId not found");
});
...
test('Validate pet structure', async () => {
  ...
  for (const key of keys) {
    if (!(key in body)) errorMessage(`Missing '${key}' in reponse`);
  }
});

test('Search pets by invalid status', async () => {
  const res = await api.get('/pet/findByStatus?status=ghosted');
  if (res.status() !== 200) errorMessage("Status non existent");
});
...
test('Get inventory', async () => {
  ...
  if (res.status() !== 200) errorMessage("Get inventory failed");
});
...
test('Place order', async () => {
  ...
  if (res.status() !== 200) errorMessage("Failed to place an order");
});
...
test('Delete order', async () => {
  ...
  if (res.status() !== 200) errorMessage("Failed to delete the order");
});

test('Get order inexistente', async () => {
  const res = await api.get('/store/order/999999');
  if (res.status() === 200) errorMessage("Got an order that should not exist");
});
...
test('Validar claves de inventario', async () => {
  ...
  for (const key of expectedKeys) {
    if (!(key in body)) errorMessage(`Missing '${key}' on inventory`);
  }
});
...
test('Create user', async () => {
  ...
  if (res.status() !== 200) errorMessage("User creation failed");
});

test('Get user by username', async () => {
  const res = await api.get(`/user/${user.username}`);
  if (res.status() !== 200) errorMessage("Missing userId or user was not created");
});

test('Update user', async () => {
  const updated = { ...user, firstName: "UpdatedSara" };
  const res = await api.put(`/user/${user.username}`, updated);
  if (res.status() !== 200) errorMessage("Update user failed");
});

test('Delete user', async () => {
  const res = await api.delete(`/user/${user.username}`);
  if (res.status() !== 200) errorMessage("Failed to delete user");
});

test('Login user', async () => {
  const res = await api.get(`/user/login?username=${user.username}&password=${user.password}`);
  if (res.status() !== 200) errorMessage("Login failed or user does not exist");
});

test('Logout user', async () => {
  const res = await api.get('/user/logout');
  if (res.status() !== 200) errorMessage("Logout failed, session may not have been active");
});
...
test('Createusuario without username', async () => {
  ...
  if (res.status() === 200) errorMessage("User without username was created");
});

test('Login with invalid credentials', async () => {
  const res = await api.get(`/user/login?username=wrong&password=fail`);
  if (res.status() === 200) errorMessage("Successful login with invalid credentials");
});